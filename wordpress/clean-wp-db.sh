...
echo; echo "Starting to backup..."; echo
...
echo 'Cleaning up transients in options table' >> ~/log/clean-wp-db.log
$(which mysql) -vvv -u$WPUSER -p$WPPASS $WPDB -e 'DELETE FROM `'$WPPREFIX'options` WHERE `option_name` LIKE ('_transient_%');' &>> ~/log/clean-wp-db.log
$(which mysql) -vvv -u$WPUSER -p$WPPASS $WPDB -e 'DELETE FROM `'$WPPREFIX'options` WHERE `option_name` LIKE ("_site_transient_%");' &>> ~/log/clean-wp-db.log
